...
// Bounded retry for a single table's read (2026-09-16) — a transient error
// (a Gateway Timeout, a momentary connection blip) used to be treated
// exactly the same as a permanent one: one failed select('*') and that
// table was immediately marked failed for the whole run. Retries a fixed,
// small number of times with a short linear backoff BEFORE giving up —
// deliberately not trying to classify "transient" vs. "permanent" from the
// Supabase/PostgREST error shape (unreliable in practice); retrying
// unconditionally costs nothing extra for a genuinely permanent failure
// (it still exhausts the bound quickly and gets marked failed) and fixes
// the common transient case for free.
export const TABLE_READ_MAX_ATTEMPTS = 3;
const TABLE_READ_RETRY_DELAY_MS = 300;
function sleep(ms) { return new Promise(resolve => setTimeout(resolve, ms)); }
async function readTableWithRetry(supabase, table) {
  let lastError = null;
  for (let attempt = 1; attempt <= TABLE_READ_MAX_ATTEMPTS; attempt++) {
    const { data, error } = await supabase.from(table).select('*');
    if (!error) return { data: data || [], error: null, attempts: attempt };
    lastError = error;
    if (attempt < TABLE_READ_MAX_ATTEMPTS) await sleep(TABLE_READ_RETRY_DELAY_MS * attempt);
  }
  return { data: null, error: lastError, attempts: TABLE_READ_MAX_ATTEMPTS };
}

// Reads every table in BACKUP_TABLES in parallel, retrying each one
// independently (see readTableWithRetry above) before giving up on it. A
// table that still fails after every retry is recorded in BOTH `warnings`
// (a human-readable line, for the existing logError()/email consumers) and
// the new `failedTables` array (a plain list of table names, for anything
// that wants to check programmatically) — and is snapshotted as an empty
// array, same as before, so a partial snapshot with the failure clearly
// marked still beats no snapshot at all for the tables that DID succeed.
//
// `complete`/`failedTables` are ALSO written into `snapshot.meta` itself
// (not just returned alongside it) — this is the actual fix for "fails
// loudly instead of silently shipping incomplete": every consumer of a
// snapshot row (the admin Restore panel, a future audit, a human reading
// the raw ops_backups.data column) can tell a genuinely-empty table apart
// from one that failed to read, without needing this function's own
// return value, which nothing but the two immediate callers ever sees.
// Deliberately stored inside the existing `data` jsonb column rather than
// as a new ops_backups column — no migration needed (rule #12's "confirmed
// applied to prod before merge" burden, with no live DB access available
// to do that from this environment — rule #11), and ops_backups.data is
// already a flexible blob every other piece of snapshot metadata
// (generatedAt, tableCounts) already lives in the same way.
export async function buildBackupSnapshot(supabase) {
  const warnings = [];
  const failedTables = [];
  const tables = {};
  await Promise.all(BACKUP_TABLES.map(async (t) => {
    const { data, error, attempts } = await readTableWithRetry(supabase, t);
    if (error) {
      warnings.push(`${t}: ${error.message} (failed after ${attempts} attempt(s))`);
      failedTables.push(t);
      tables[t] = [];
      return;
    }
  ...
  return {
    ...
    snapshot: {
      tables,
      meta: { generatedAt: new Date().toISOString(), tableCounts, complete, failedTables },
    },
  };
}
