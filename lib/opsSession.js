...
// Exported (2026-09-12) so a caller with its own expensive parallel work to
// do — currently only api/ops-state.js's 21-table fan-out — can call this
// directly and run it CONCURRENTLY with that work instead of sequentially in
// front of it (see ops-state.js's handler). requireSession() itself is
// UNCHANGED and still calls this the same (sequential) way for every other
// caller — this export doesn't alter requireSession()'s own behavior at all.
...
// Restriction project (see CLAUDE.md): every non-super admin — not just the
// three specialized manager levels this used to single out — is now
// hard-blocked from editing user/admin records and from seeing pay rates.
// Previously this allowlisted everything except RESTRICTED_MANAGER_LEVELS,
// which left the plain 'admin' level (and any future non-super level) with
// full edit/pay-rate access by omission — the exact gap this closes. Both of
// this function's only two call sites (api/ops-sync.js's user-write gate,
// api/ops-state.js's payRate/hours read-stripping) now correctly apply to
// every admin who isn't super/owner, with zero further change needed at
// either site.
export function canEditUsers(session) {
  return tierOf(session) === 'super';
}
