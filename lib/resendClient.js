...
export function buildEmailHtml({ name, title, body, link }) {
  ...
  // Falls back to APP_URL (set in Vercel env to the complete production
  // link, e.g. https://opshub.weblightmedia.com/user — NOT appended to
  // here) rather than a hardcoded URL — every server-fired notification
  // email (assignment/done/comment/overdue/time-off/message, from
  // api/ops-sync.js) passes link:'' and lands on this fallback, so a bare
  // domain literal here silently goes stale the next time the production
  // domain changes.
