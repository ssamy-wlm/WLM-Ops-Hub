...
export function buildEmailHtml({ name, title, body, link }) {
  ...
  // Falls back to APP_BASE_URL (set in Vercel env to the production domain)
  // rather than a hardcoded URL — every server-fired notification email
  // (assignment/done/comment/overdue/time-off/message, from api/ops-sync.js)
  // passes link:'' and lands on this fallback, so a bare domain literal here
  // silently goes stale the next time the production domain changes.
  const appBaseUrl = (process.env.APP_BASE_URL || 'https://opshub.weblightmedia.com').replace(/\/+$/, '');
  const safeLink = escHtml(link || `${appBaseUrl}/user`);
  return `
